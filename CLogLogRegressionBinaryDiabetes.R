...
fitted.model <- glm(diabetes ~ gender + smoking_history + age + hypertension + bmi + HbA1c_level + blood_glucose_level, 
                    data = diabetes.data, family = binomial(link = cloglog))
...
#checking model fit
null.model<- glm(diabetes.data$diabetes ~ 1, family=binomial(link=cloglog))
...
ggplot(logit_df, aes(x = reorder(Feature, abs(Estimate)), y = Estimate, fill = logP)) +
  ...
  labs(
    title = "Complementary Log Log Regression Coefficients with Significance",
    x = "Feature",
    y = "Coefficient (Slope)"
  ) +
